-- The stuff you get for buying the game in the GOG or by linking your GOG account.
return {
	Inventory = {
		-- BLACK UNICORN / Weapon / Melee / Blade / Rare
		-- The razor-like blade is as merciless as death itself, the guard is fashioned from premium materials and the handle is crafted so exquisitely it belongs in a museum. Most importantly, it remains light and nimble to wield because it was forged DRM-free.
		{ id = "Items.Preset_Katana_GoG" }, 

		-- BLACK UNICORN / Weapon / Melee / Blade / Epic
		-- The razor-like blade is as merciless as death itself, the guard is fashioned from premium materials and the handle is crafted so exquisitely it belongs in a museum. Most importantly, it remains light and nimble to wield because it was forged DRM-free.
		{ id = "Items.Preset_Katana_GoG_Epic" }, 

		-- BLACK UNICORN / Weapon / Melee / Blade / Legendary
		-- The razor-like blade is as merciless as death itself, the guard is fashioned from premium materials and the handle is crafted so exquisitely it belongs in a museum. Most importantly, it remains light and nimble to wield because it was forged DRM-free.
		{ id = "Items.Preset_Katana_GoG_Legendary" }, 

		-- GALAXY T-SHIRT / Clothing / Inner Torso / Rare
		-- Find your place amongst the stars.
		{ id = "Items.GOG_Galaxy_TShirt" }, 

		-- GALAXY T-SHIRT / Clothing / Inner Torso / Epic
		-- Find your place amongst the stars.
		{ id = "Items.GOG_Galaxy_TShirt_Epic" }, 

		-- GALAXY T-SHIRT / Clothing / Inner Torso / Legendary
		-- Find your place amongst the stars.
		{ id = "Items.GOG_Galaxy_TShirt_Legendary" }, 

		-- WOLF SCHOOL JACKET / Clothing / Outer Torso / Rare
		-- Because the world in 2077 could still use a monster hunter.
		{ id = "Items.GOG_DLC_Jacket" }, 

		-- WOLF SCHOOL JACKET / Clothing / Outer Torso / Epic
		-- Because the world in 2077 could still use a monster hunter.
		{ id = "Items.GOG_DLC_Jacket_Epic" }, 

		-- WOLF SCHOOL JACKET / Clothing / Outer Torso / Legendary
		-- Because the world in 2077 could still use a monster hunter.
		{ id = "Items.GOG_DLC_Jacket_Legendary" }, 

		-- WOLF SCHOOL T-SHIRT / Clothing / Inner Torso / Rare
		-- When you have to choose between neokitsch and neomilitaristic but can't make up your mind.
		{ id = "Items.GOG_DLC_TShirt" }, 

		-- WOLF SCHOOL T-SHIRT / Clothing / Inner Torso / Epic
		-- When you have to choose between neokitsch and neomilitaristic but can't make up your mind.
		{ id = "Items.GOG_DLC_TShirt_Epic" }, 

		-- WOLF SCHOOL T-SHIRT / Clothing / Inner Torso / Legendary
		-- When you have to choose between neokitsch and neomilitaristic but can't make up your mind.
		{ id = "Items.GOG_DLC_TShirt_Legendary" }, 
	},
}